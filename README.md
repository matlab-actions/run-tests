...
`strict`                    | (Optional) Whether to apply strict checks when running the tests. For example, the action generates a qualification failure if a test issues a warning.
...
`output-detail`             | (Optional) Display level for event details produced by the test run, specified as `None`, `Terse`, `Concise`, `Detailed`, or `Verbose`.
`logging-level`             | (Optional) Maximum verbosity level for logged diagnostics included for the test run, specified as `None`, `Terse`, `Concise`, `Detailed`, or `Verbose`.
