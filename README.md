...
Use the **Run MATLAB Tests** action to automatically run tests authored using the MATLAB unit testing framework or Simulink Test&trade;. To use the action, you need MATLAB R2013b or a later release. Some of the optional action inputs require newer MATLAB releases or additional toolboxes.
...
| `code-coverage-html`         | <p>(Optional) Location to write the code coverage results in HTML format, specified as a folder path relative to the project root folder.</p><p>The action uses the `code-coverage-metrics` input to determine which coverage metrics to include in the report. If you do not specify `code-coverage-metrics`, the action uses the default value (`auto`).</p><p>**Example:** `code-coverage-html: code-coverage`</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `code-coverage-cobertura`    | <p>(Optional) Location to write the code coverage results in Cobertura XML format, specified as a file path relative to the project root folder.</p><p>The action uses the `code-coverage-metrics` input to determine which coverage metrics to include in the report. If you do not specify `code-coverage-metrics`, the action uses the default value (`auto`). The Cobertura XML format supports at most line and decision (branch) coverage.</p><p>**Example:** `code-coverage-cobertura: code-coverage/coverage.xml`</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `code-coverage-metrics`      | <p>(Optional) Code coverage metrics to collect, specified as one or more of these values separated by spaces:<ul><li>`statement` — Statement and function coverage</li><li>`decision` (requires MATLAB Test&trade;) — Decision coverage</li><li>`condition` (requires MATLAB Test) — Condition coverage</li><li>`mcdc` (requires MATLAB Test) — Modified condition/decision coverage (MC/DC)</li><li>`type-size` (requires MATLAB Test) — Data type and size coverage (*since R2026b*)</li></ul>These structural coverage metrics (`statement`, `decision`, `condition`, and `mcdc`) are hierarchical, meaning that each metric includes all lower metrics. For more information about coverage metrics, see [Types of Code Coverage for MATLAB Source Code](https://www.mathworks.com/help/matlab-test/ug/types-of-code-coverage-for-matlab-source-code.html).</p><p>By default, the value of `code-coverage-metrics` is `auto`. If you have a MATLAB Test license, `auto` collects all structural coverage metrics for the HTML format, and line and decision (branch) coverage for the Cobertura XML format. If you do not have a MATLAB Test license, `auto` collects statement and function coverage for the HTML format, and line coverage for the Cobertura XML format.</p><p>**Example:** `code-coverage-metrics: decision`<br/>**Example:** `code-coverage-metrics: mcdc type-size`</p> |
...
If the `generate-summary` input is `true`, you can view test results and code coverage metrics for your source code in the GitHub job summary.
...
After your workflow runs, the GitHub job summary displays the number of tests in the test suite, as well as the number of tests that passed, failed, were incomplete (due to assumption failure), and did not run (due to fatal assertion failure). Additionally, the **All tests** table in the summary includes a row for each executed test file. Each table row displays the status of a test file, its name, and the time it took to run. You can hover over a filename in the table to see the file path within your project.
...
> [!TIP]
> You also see your MATLAB and Simulink test results in the GitHub job summary when you take any of these actions:
>
...
The **MATLAB Code Coverage** table in the GitHub job summary displays the structural code coverage metrics for the MATLAB source code specified by the `source-folder` input of the **Run MATLAB Tests** action. To generate the coverage summary table, you must not specify the `code-coverage-html` or `code-coverage-cobertura` action inputs.

The action uses the `code-coverage-metrics` input to determine which coverage metrics to include in the summary. By default, the value of `code-coverage-metrics` is `auto`. If you have a MATLAB Test license, `auto` collects all structural coverage metrics: statement, function, decision, condition, and modified condition/decision coverage (MC/DC). If you do not have a MATLAB Test license, `auto` collects statement and function coverage.

To exclude the coverage summary table without affecting the test results summary, specify `code-coverage-metrics` as an empty value (`''`).
