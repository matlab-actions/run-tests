...
| `code-coverage-metric-level` | <p>(Optional) Location to write the code coverage results in Cobertura XML format, specified as a file path relative to the project root folder.</p><p>**Example:** `code-coverage-metric-level: statement`</p> |
...
The **MATLAB Code Coverage** table on the workflow run summary page displays the collected code coverage metrics for the source code specified by the `source-folder` input of the **Run MATLAB Tests** action. For the action to generate this table, the following requirements must be met:
