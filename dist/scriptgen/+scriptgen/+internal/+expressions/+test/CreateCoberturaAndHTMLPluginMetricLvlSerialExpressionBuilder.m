classdef CreateCoberturaAndHTMLPluginMetricLvlSerialExpressionBuilder < scriptgen.expressions.test.CreateCoberturaAndHTMLCodePluginSerialExpressionBuilder ...
    ...
    methods
        function expression = build(obj)
            ...
            if isempty(obj.Metrics)
                ...
                text = sprintf('CodeCoveragePlugin.forFolder({%s}, ''IncludingSubfolders'', true, ''MetricLevel'', %s, ''Producing'', [CoberturaFormat(%s) CoverageReport(%s)])', source, metrics, obj.CoberturaFilePath, obj.HTMLFolderPath);
            end
