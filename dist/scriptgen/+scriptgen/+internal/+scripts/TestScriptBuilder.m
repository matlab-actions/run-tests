    ...
    methods
        function script = build(obj)
            ...
            sequences(end+1) = testCodeProvider.createSequence('CreateTestSuite', ...
                ...
                'SelectByName', quoteCell(splitName(obj.SelectByName)));
    ...
    methods (Access = private)
        ...
        function s = buildCreateTestRunnerSequence(obj, provider, runnerName, useParallel)
            ...
            if isscalar(metrics) && strcmpi(metrics{1}, 'auto')
                currentVersion = Version.forCurrentRelease();
                if any(strcmp({ver().Name}, 'MATLAB Test')) && license('test', 'MATLAB_Test')
                    if currentVersion >= Version.forRelease('R2023a')
                        metrics = {'''mcdc'''};
                    else
                        metrics = {'''statement'''};
                    end
