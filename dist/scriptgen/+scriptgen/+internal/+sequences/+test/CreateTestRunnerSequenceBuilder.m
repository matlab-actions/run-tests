    ...
    methods
        function sequence = build(obj)
            ...
            statements = testCodeProvider.createStatement('CreateTestRunner', ...
                ...
                'HasSimulinkTestResults', ~isempty(obj.SimulinkTestResults), ...
            ...
            % If a default testrunner is used, check if we still need to add TestManagerResultsPlugin again for PDF/HTML reports
            if any(~cellfun(@isempty, strfind({statements.Contents}, 'testrunner')))
                tempRunner = testrunner;
                if any(arrayfun(@(p) isa(p, 'sltest.plugins.TestManagerResultsPlugin'), tempRunner.Plugins))
                    hasSimTest = true;
                end
            end
            
            if (hasPDF || hasHTML) && ~hasSimTest
                % Do not warn if we cannot add Simulink Test plugin because the user did
                % not explicitly request it.
                warningState = warning('off', 'scriptgen:featureNotAvailable');
                restoreState = onCleanup(@()warning(warningState));
